...
overdue: tasks.filter(t => !t.completed && new Date(t.due_date || t.dueDate) < now).length
      })
    } catch (error) {
  ...
  return (
    ...
    <div className="max-w-7xl mx-auto">
        ...
        <div
            ...
            <Button onClick={handleAddTask} icon="Plus">Add Task
                          </Button>
        </div>
        {/* Task Stats */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-6">
                ...
                <div className="space-y-2">
                    ...
                    ${filter === option.value ? "bg-primary text-white" : "text-gray-600 hover:bg-gray-100 hover:text-gray-900"}
                  `}>
                        <span>{option.label}</span>
                        <span
                            className={`
                    px-2 py-0.5 rounded-full text-xs
                    ${filter === option.value ? "bg-white/20 text-white" : "bg-gray-100 text-gray-500"}
                  `}>
                            {option.count}
                        </span>
                    </button>)}
...
useEffect(() => {
    if (task) {
      setFormData({
        ...task,
        due_date: new Date(task.due_date || task.dueDate).toISOString().split('T')[0]
      })
    } else {
...
label="Related To (Customer/Lead ID)"
              ...
              onChange={(e) => setFormData(prev => ({ ...prev, related_to: e.target.value }))}
